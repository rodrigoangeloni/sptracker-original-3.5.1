...
ptracker_lib/stdlib64/CreateFileHook.dll""".split("\n")
    ...
    # Ya no se genera ptracker-server-dist.py, se usa directamente ptracker-server.py

    os.environ['PYTHONPATH'] = r"..\system;..\..\system"
    print("------------------- Building ptracker.exe -------------------------------")
    # Usar el archivo .spec existente en lugar de generar uno nuevo
    assert 0 == os.system(r'.venv\\Scripts\\python -m PyInstaller --clean -y ptracker.spec')
...
if not ptracker_only:
    ...
    assert 0 == os.system("pyinstaller --name stracker --clean -y --onefile --exclude-module http_templates --hidden-import cherrypy.wsgiserver.wsgiserver3 --hidden-import psycopg2 --additional-hooks-dir=pyinstaller-hooks --path .. stracker.py")
    ...
    assert 0 == os.system("pyinstaller --clean -y --onefile --path .. stracker-packager.py")
